...
function getRandomColor() {
    // Genera un número aleatorio de 0 a FFFFFF (16777215)
    return Math.random() * 0xffffff;
}


document.addEventListener('DOMContentLoaded', () => {
    const colorButton = document.getElementById('cambiarColorBtn');
    
    colorButton.addEventListener('click', () => {
        // Obtenemos un nuevo color aleatorio
        const newColor = getRandomColor();
        
        // Asignamos el nuevo color al material de la copa
        leavesMaterial.color.set(newColor);
        
        // Forzamos el renderizado para que el cambio sea inmediato
        renderer.render(scene, camera);
    });
});