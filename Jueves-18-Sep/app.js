...
camera.position.set(0, 5, INITIAL_CAMERA_Z);
camera.lookAt(0, 0, 0);
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    zoomInButton.addEventListener('click', () => {
        // Mueve la cámara -3 unidades Z desde la posición actual (hacia el árbol)
        if (camera.position.z > 2) { // Evita atravesar el árbol
    ...
    zoomOutButton.addEventListener('click', () => {
        // Mueve la cámara +3 unidades Z desde la posición actual (alejándose)
        gsap.to(camera.position, {
            z: `+=${ZOOM_STEP}`,
            duration: 0.8,
            ease: 'power2.out'
